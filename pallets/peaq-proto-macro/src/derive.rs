...
use crate::{
	attr::parse_proto_attrs,
	codegen::{generate_enum, generate_message, generate_oneof_message},
};
...
pub fn expand(input: TokenStream) -> TokenStream {
	...
	let proto_name = item_attrs.get("name").cloned().unwrap_or_else(|| rust_name.to_string());

	let snippet: String = match &ast.data {
		Data::Struct(data) => generate_message(&proto_name, &data.fields),
		Data::Enum(data) => {
			let has_data = data.variants.iter().any(|v| !matches!(v.fields, Fields::Unit));
			if has_data {
	...
	let const_ident = syn::Ident::new(&format!("PROTO_SNIPPET_{}", rust_name), rust_name.span());
