...
use crate::attr::{parse_proto_attrs, sanitize_field_name, to_snake_case, to_upper_snake_case};
...
pub fn generate_message(proto_name: &str, fields: &Fields) -> String {
	...
	for field in &named.named {
		...
		let field_number =
			attrs.get("field").and_then(|v| v.parse::<u32>().ok()).unwrap_or_else(|| {
				panic!(
		...
		lines.push(format!("  {}{} {} = {};", prefix, proto_type, clean_name, field_number));
	}
...
pub fn generate_enum(proto_name: &str, data: &DataEnum) -> String {
	...
	for variant in &data.variants {
		...
		let value = attrs.get("value").and_then(|v| v.parse::<u32>().ok()).unwrap_or_else(|| {
			panic!(
...
pub fn generate_oneof_message(
	...
	let mut lines =
		vec![format!("message {} {{", proto_name), format!("  oneof {} {{", oneof_field)];
