...
impl<T: Ord + Clone, S: Get<u32>> OrderedSet<T, S> {
	...
	/// given element.
	///
	/// 1. If the value is found, then Result::Ok is returned, containing the index of the matching
	///    element.
	/// 2. If the value is not found, then Result::Err is returned, containing the index where a
