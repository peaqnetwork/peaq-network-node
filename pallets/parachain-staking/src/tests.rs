...
fn check_claim_block_normal_wo_delegator() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let total_stake_in_session: u128 = 2 * (stake) + 2 * (stake * 2);
			let collator_1_percentage =
				Perquintill::from_rational(2 * stake, total_stake_in_session);
			let collator_2_percentage =
				Perquintill::from_rational(2 * (stake * 2), total_stake_in_session);

			// verify rewards for round 0, session 1
			roll_to(10, authors.clone());
			assert_eq!(
				Balances::free_balance(1),
				collator_1_percentage * BLOCK_REWARD_IN_GENESIS_SESSION + origin_balance
			);
			assert_eq!(
				Balances::free_balance(2),
				collator_2_percentage * BLOCK_REWARD_IN_GENESIS_SESSION + origin_balance
			);

			// verify rewards for round 1, session 2
			// Cross session but only 1 collator is selected
			roll_to(15, authors.clone());
			assert_eq!(
				Balances::free_balance(1),
...
fn check_claim_block_normal_wi_delegator() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let collator_1_total_stake = stake + (5 * stake) + (6 * stake);
			let collator_2_total_stake = (2 * stake) + (7 * stake);
			let total_stake_in_round_0 =
				(2 * collator_1_total_stake) + (2 * collator_2_total_stake);
			let collator_1_percentage =
				Perquintill::from_rational(2 * stake, total_stake_in_round_0);
			let collator_2_percentage =
				Perquintill::from_rational(2 * (2 * stake), total_stake_in_round_0);
			let delegator_5_percentage =
				Perquintill::from_rational(2 * 5 * stake, total_stake_in_round_0);
			let delegator_6_percentage =
				Perquintill::from_rational(2 * 6 * stake, total_stake_in_round_0);
			let delegator_7_percentage =
				Perquintill::from_rational(2 * 7 * stake, total_stake_in_round_0);
			...
			assert_eq!(
				Balances::free_balance(5),
				delegator_5_percentage * BLOCK_REWARD_IN_GENESIS_SESSION + origin_balance
			);
			assert_eq!(
				Balances::free_balance(6),
				delegator_6_percentage * BLOCK_REWARD_IN_GENESIS_SESSION + origin_balance
			);
			...
			// Delegator 5's expected balance 10000000000000000003035
			// assert_eq!(
			// 	Balances::free_balance(5),
			// 	delegator_5_percentage * BLOCK_REWARD_IN_GENESIS_SESSION +
...
fn check_collator_kickout() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(CandidatePool::<Test>::count(), 6);
			assert_eq!(collator_blocks.len(), 2);
			assert!(events().contains(&Event::CollatorSlashed(3, 1)));
			roll_to(20, authors.clone());

			assert_eq!(Balances::locks(1).first().unwrap().amount, 10);
			assert_eq!(Balances::locks(2).first().unwrap().amount, 10);
			assert_eq!(Balances::locks(3).first().unwrap().amount, 9);
			assert_eq!(Balances::locks(4).first().unwrap().amount, 9);
			assert_eq!(Balances::locks(5).first().unwrap().amount, 9);
			assert_eq!(Balances::locks(6).first().unwrap().amount, 10);
			assert_eq!(CandidatePool::<Test>::get(3).unwrap().total, 9);
			assert!(TopCandidates::<Test>::get().contains(&Stake { owner: 3, amount: 9 }));
			assert_eq!(StakePallet::selected_candidates().contains(&3), false);
		});
}

#[test]
fn check_non_active_collator_no_slash() {
	ExtBuilder::default()
		.with_balances(vec![(1, 100), (2, 100), (3, 100), (4, 100), (5, 100), (6, 100)])
		.with_collators(vec![(1, 100), (2, 100), (3, 100), (4, 50), (5, 50), (6, 50)])
		.build()
		.execute_with(|| {
			assert_eq!(StakePallet::slashing_factor(), Permill::from_percent(10));
			let authors: Vec<Option<AccountId>> = (0u64..=22).map(|i| Some(i % 2 + 1)).collect();
			...
			assert_eq!(Balances::locks(2).first().unwrap().amount, 100);
			assert_eq!(Balances::locks(3).first().unwrap().amount, 73);
			assert_eq!(Balances::locks(4).first().unwrap().amount, 50);
			assert_eq!(Balances::locks(5).first().unwrap().amount, 50);
			assert_eq!(Balances::locks(6).first().unwrap().amount, 50);
			assert_eq!(CandidatePool::<Test>::get(3).unwrap().total, 73);
			assert!(TopCandidates::<Test>::get().contains(&Stake { owner: 3, amount: 73 }));
		});
}
