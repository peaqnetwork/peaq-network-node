...
fn asset_id_to_location() {
	...
	assert_eq!(AssetLocationIdConverter::<AssetId, AssetLocationMapper>::convert_back(&0), None);
}
...
fn fixed_rate_of_foreign_asset_buy_is_ok() {
	...
	let expected_execution_fee =
		execution_fee(weight, ExecutionPayment::get_units_per_second(PARENT).unwrap());
