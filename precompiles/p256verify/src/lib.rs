...
// Ported for peaq from Moonbeam (moonbeam-foundation/moonbeam,
// precompiles/p256verify). Both projects are GPL-3.0-or-later.
//
...
impl<W: Get<Weight>, G: Get<u64>> P256Verify<W, G> {
	...
	fn verify_from_input(input: &[u8]) -> Option<()> {
		...
		// (0x04) prefix indicates the public key is in its uncompressed form
...
impl<W: Get<Weight>, G: Get<u64>> Precompile for P256Verify<W, G> {
	fn execute(handle: &mut impl PrecompileHandle) -> PrecompileResult {
		...
		Ok(PrecompileOutput { exit_status: ExitSucceed::Returned, output: result.to_vec() })
	}
}
...
mod tests {
	...
	fn test_valid_signature() {
		// RIP-7212 test vectors: 2 valid, 3 invalid.
		let inputs = vec![
			(
				true,
				hex!(
		...
		for input in inputs {
			...
			match (input.0, P256Verify::<DummyWeight, DummyGas>::execute(&mut handle)) {
