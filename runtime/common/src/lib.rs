...
pub fn local_peaq_asset_location(key: PeaqAssetId) -> Option<MultiLocation> {
	Some(MultiLocation::new(0, X1(Junction::from(BoundedVec::try_from(key.encode()).ok()?))))
}
...
//         log::error!(
//             "Show me the log: PeaqAssetIdConvert, {:?}: {:?}",
//             "now", now
//         );
//         log::error!(
//             "Show me the log: PeaqAssetIdConvert, {:?}: {:?}",
...
//         log::error!(
//             "Show me the log: PeaqAssetIdConvert, {:?}: {:?}? {:?}",
//             "bnc", bnc_location, bnc_location == now
//         );
//         log::error!(
//             "Show me the log: PeaqAssetIdConvert, {:?}: {:?} {:?}",
//             "peaq", peaq_location, peaq_location == now
//         );
//
//
// 		if now == peaq_location {
