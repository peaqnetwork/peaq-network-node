...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: Cow::Borrowed("peaq-node-krest"),
	impl_name: Cow::Borrowed("peaq-node-krest"),
	authoring_version: 1,
...
/// We allow `Normal` extrinsics to fill up the block up to 90%, the rest can be used by
...
/// fresh `--chain krest-local` networks (see `node/src/parachain/krest_chain_spec.rs`).
/// Krest keeps the exact same six pots and shares the legacy config used to have.
///
